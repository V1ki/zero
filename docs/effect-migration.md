...
| packages/core | 19450 | 最高 | ✅ 审计判定 + 定时器卫生修复 | 逐点核验 9 处定时器：**全部为请求级 timeout/backoff race**（fetch/bash/grep/x-search/compress/session-drain/waitAgents/background-threshold），包内零 interval、零自有长活循环。工具运行时 fiber 化维持 defer（fiber 包装无法取消 Promise 化的 Bun.spawn——真实取消仍靠工具内 AbortController/proc.kill 闩；grep 超时部分结果语义与 race 输家丢弃冲突），重启条件不变。审计顺手修复三处真实泄漏：`background-tool-tasks.run()`（每次快速工具调用悬挂最长 60s 的 threshold timer）、session manager drain race、agent-control waitAgents race——赢方先行时定时器从不清除，正是 Effect 可中断 race 所防的失效模式，此处以 Promise 卫生修复替代 |
