...
- 消息已被压缩移除的旧 turn 不会变成空白：Trajectory 会把这些 turn 的未配对请求渲染成 request-only 活动行（工具调用行 + assistant 响应首行），模型、tokens、耗时通过 request 编号在请求详情面板查看。turn 开头的用户输入来自链根请求日志里的 user prompt，渲染为 USER 行；Initial System Prompt 记录落在第一个实际渲染的 turn（含压缩 era turn）。turn 形状保持 user → gateway → (context) → steps：layer-1 memory retrieval 在用户输入之后、该 turn 首个模型请求之前触发（span 起点早于请求起点，因此按"下一个请求所属 turn"归因）。memory retrieval agent 这个旁路调用本身始终是 GATEWAY 记录，Payload/Result 里是触发消息、生成的 queries、执行的搜索与被选中注入的 memories；若选中了记忆，紧随其后会插入一条独立的 `memory context · N memories injected` 记录，它才是进入模型上下文的 CONTEXT 消息：正文是运行时真正注入的原文（请求日志 `memoryInjections[].formattedText`，按被选中 memory 的 id 匹配），详情面板可看全文；请求日志没有对应注入条目时回退为选中 memory 的标题/类型/分数列表。layer-2 memory hint（工具失败后触发）同样处理：检索 span 投影为 `memory hint retrieval` GATEWAY 记录，锚定在 ledger 里被注入的 hint 消息（或压缩 era 中下一个请求行）之前；注入的 `<memory_hint>` 消息重塑为 `memory hint · N memories injected` CONTEXT 记录，保留注入原文。
