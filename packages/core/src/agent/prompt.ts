...
export function buildToolRulesBlock(tools: ToolDefinition[]): string {
  const toolRuleMap: Record<string, string> = {
    ...
    source_card:
      'Source Card：当用户请求像是在查询已知外部数据源（股票行情、邮箱状态、系统数据、长期可复用数据源）时，优先调用 source_card list 做 preflight；若列表里有匹配项，必须再调用 source_card get 读取 sourceDoc 这份类似 SKILL.md 的 Markdown 使用说明，然后优先按 sourceDoc 选择 URL、CLI、字段、fallback 和安全边界。Source Card 不是执行器，不代表已经能自动获取数据；active/public Source Card 可继续使用 fetch/bash/browser 等前台工具，但必须遵守 sourceDoc 中写明的安全边界；private/restricted/draft Source Card 要说明阻塞原因，不要绕过 Source Card 去后台读取。若任务中发现了 sourceDoc 未记录但成功的新方法，结束前用 source_card generate_draft/validate_draft 沉淀草稿，不要只写 memory。',
