...
export const CONTEXT_COMPACTION_SYSTEM_PROMPT = [
  ...
  '必须覆盖工具调用；大型 tool_result 会以截断形式进入 prompt，必要时引用 raw evidence path。',
...
function withTimeout<T>(operation: Promise<T>, timeoutMs: number, message: string): Promise<T> {
  ...
  // Promise.race observes the operation even when the timeout wins, so a late
  // rejection from the underlying request cannot surface as unhandled.
  operation.catch(() => {})
...
核心目标：
- 压缩结果会在 prompt 主线上替代这些原始消息；大型 tool IO 以截断原文 + evidence path 进入 prompt。
- evidence path 是审计和回看来源；如果原文被截断且仍需回看，需要在 topic evidence 中保留对应 K 引用和 path。
...
async function requestContextCompactionModel(params: {
  ...
  const response = await withTimeout(
    params.adapter.complete(request),
    CONTEXT_PARAMS.history.compactionModelTimeoutMs,
    `context_compaction model request timed out after ${CONTEXT_PARAMS.history.compactionModelTimeoutMs}ms`,
  )
