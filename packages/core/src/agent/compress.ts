...
type ContextCompactionTracer = Pick<Tracer, 'startSpan' | 'updateSpan' | 'endSpan' | 'getSpan'> & {
...
export const CONTEXT_COMPACTION_SYSTEM_PROMPT = [
  ...
  '必须覆盖工具调用；tool_result 原文会作为 covered_messages 输入，请直接分析这些原文，不要只依赖摘要。',
...
核心目标：
- 压缩结果会在 prompt 主线上替代这些原始消息；tool_result 原文已经进入 covered_messages，请直接基于原文理解和归纳。
- evidence path 只是审计和回看来源，不是摘要的替代品；不要因为有 evidence path 就跳过 tool_result 原文。
- 不要把 tool_use / tool_result 原文简单堆叠进 summary；要解释为什么这些工具被调用、实际看了/写了/运行了什么、学到了什么。
