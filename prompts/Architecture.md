...
.zero/
  ...
  └─ workspace/          # AI 的工作目录
      ...
      │   ├─ USER.md     #     用户画像
      │   └─ TOOLS.md    #     工具环境说明
      └─ shared/         #   共享目录，最终产出物留档于此，供用户和其他 Agent 访问
```

每个 Agent 只在自己的目录下工作，临时文件和草稿不影响其他 Agent。需要交付的产出物放到 `shared/` 留档，同时必须在对话中输出完整内容或关键摘要——不能只写文件不回复，用户可能在手机上操作无法直接查看文件。`shared/` 遵循 Write/Edit 的文件锁。
