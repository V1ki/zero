  ...
  return (
    <div className="px-5 py-4 max-w-[1680px] mx-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-[11px] text-[var(--color-text-disabled)]">
                run.log · {firstTs ? new Date(firstTs).toLocaleString() : '-'} to{' '}
      ...
      <div className="mb-4 grid grid-cols-2 gap-2 md:grid-cols-5">
        <Metric label="Loaded" value={entries.length.toLocaleString()} />
        <Metric label="Visible" value={filtered.length.toLocaleString()} />
        <Metric label="Errors" value={(levelCounts.error ?? 0).toLocaleString()} tone="error" />
        <Metric
          label="Raw LLM"
          value={`${countEvent(entries, 'llm_request.raw_request')}/${countEvent(
            entries,
            'llm_request.raw_response',
          )}`}
        />
        <Metric label="Tool Logs" value={countPrefix(entries, 'tool_call.').toLocaleString()} />
      </div>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2 border-y border-[var(--color-border)] py-3">
        ...
        <select
          value={level}
          onChange={(e) => setLevel(e.target.value as RunLogLevel | 'all')}
        ...
        <select
          value={timeOrder}
          onChange={(e) => {
            setTimeOrder(e.target.value as TimeOrder)
            setSelectedKey(null)
          }}
          ...
          {loading ? (
            ...
            <div className="max-h-[calc(100vh-330px)] overflow-auto">
              {filtered.map((entry) => {
                ...
                const selected = selectedEntry && getEntryKey(selectedEntry) === key
          ...
          {selectedEntry ? (
            <div className="max-h-[calc(100vh-330px)] overflow-auto p-3">
              <div className="mb-3 space-y-1 text-[11px]">
                <InspectorRow label="event" value={selectedEntry.event} />
                <InspectorRow label="time" value={new Date(selectedEntry.ts).toLocaleString()} />
                <InspectorRow label="span" value={selectedEntry.spanId ?? '-'} />
                <InspectorRow
                  label="agent"
                  value={selectedEntry.agentName ?? selectedEntry.name ?? '-'}
                />
              </div>
