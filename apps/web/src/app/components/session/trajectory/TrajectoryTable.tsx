...
import { indexSessionRequestsBySeq } from './request-numbering'
...
import { cellBadgeKind, formatElapsedSeconds, trajectoryRecordId } from './trajectory-record'
import { groupTrajectoryVirtualRows, trajectoryVirtualRecordKey } from './trajectory-virtual-rows'
import type { TrajectoryVirtualRow } from './trajectory-virtual-rows'
import type { AssistantRequestConfig, ConversationPromptSnapshot } from './types'
...
function useStableVirtualRowStructure(
  ...
  const structure =
    cache.current.structure.length === rows.length &&
...
function AssistantTimingPanel({ metrics }: { metrics: AssistantMetricDetail }) {
  return (
    <dl className={css.overview}>
      <div>
        <dt>Started</dt>
        <StartedAtValue timestamp={metrics.stepStartTime} />
      </div>
      <div>
        <dt>Total duration</dt>
        <dd>{totalTime(metrics)}</dd>
      </div>
      <div>
        <dt>TTFT</dt>
        <dd>{ttft(metrics)}</dd>
      ...
      <div>
        <dt>Generation</dt>
        <dd>{generationTime(metrics)}</dd>
      </div>
      <div>
        <dt>Throughput</dt>
        <dd>{throughput(metrics)}</dd>
      </div>
    </dl>
  )
}
...
function indexRequestNumbers(
  ...
  const boundaryRecords = records
    .filter(
      (record) =>
        boundaries.get(requestKey(record.turn, record.group)) === record.cell.index &&
...
function summarizeTurn(records: readonly TableRecord[]): string {
  const steps = new Set(
    records.map((record) => record.group).filter((group) => group.startsWith('Step ')),
  ).size
...
function collapseTurnRecords(
  ...
  return records.flatMap((record) => {
    ...
    const contentRecords = turnRecords.filter(
      (candidate) => candidate.cell.requestOnly !== true && candidate.cell.kind !== 'system',
    )
...
function collapseAssistantRecords(
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    out.push(record)
    if (record.cell.kind !== 'message' || !collapsedAssistants.has(trajectoryRecordId(record.cell)))
      continue
    const calls: TableRecord[] = []
...
function TokenRows({ cell }: { cell: TrajectoryCellProps }) {
  const content =
    cell.output !== undefined && cell.think !== undefined
...
/** Token consumption of a gate (task-closure classifier) call. */
function GateTokenRows({ cell }: { cell: TrajectoryCellProps }) {
  const content =
    cell.output !== undefined && cell.think !== undefined
      ? Math.max(0, cell.output - cell.think)
      : undefined
  return (
    <>
      <div>
        <dt>Tokens</dt>
        <dd>{cell.output === undefined ? '—' : `${cell.output.toLocaleString()} tok`}</dd>
      </div>
      {cell.input !== undefined && (
        <div className={css.requestTokenDetail}>
          <dt>Input</dt>
          <dd>{cell.input.toLocaleString()} tok</dd>
        </div>
      )}
      {cell.cacheRead !== undefined && (
        <div className={css.requestTokenDetail}>
          <dt>Cache read</dt>
          <dd>{cell.cacheRead.toLocaleString()} tok</dd>
        </div>
      )}
      {cell.cacheWrite !== undefined && (
        <div className={css.requestTokenDetail}>
          <dt>Cache write</dt>
          <dd>{cell.cacheWrite.toLocaleString()} tok</dd>
        </div>
      )}
      {cell.think !== undefined && (
        <div className={css.requestTokenDetail}>
          <dt>Reasoning</dt>
          <dd>{cell.think.toLocaleString()} tok</dd>
        </div>
      )}
      {content !== undefined && (
        <div className={css.requestTokenDetail}>
          <dt>Content</dt>
          <dd>{content.toLocaleString()} tok</dd>
        </div>
      )}
    </>
  )
...
function UsageRows({ usage }: { usage: TrajectoryUsage | undefined }) {
  ...
  const otherOutput =
    usage.output !== undefined && usage.reasoning !== undefined
...
function MessageSource({ record }: { record: TableRecord }) {
  ...
  return <JsonTree data={data} label="Message source JSON" className={css.jsonPayload} />
}

function isMarkdownRecord(record: TableRecord): boolean {
  return (
    record.cell.kind === 'user' || record.cell.kind === 'context' || record.cell.kind === 'message'
  )
}

/**
 * Gate records (task closure) captured a question/answer pair; they render
 * tool-style with separate Payload (question) and Result (answer) tabs.
 * @param record - Ledger record being inspected.
 * @returns true when the record carries gate output detail.
 */
function isGateQaRecord(record: TableRecord): boolean {
  return record.cell.kind === 'context' && record.cell.outputDetail !== undefined
}

function parentRecords(records: readonly TableRecord[], record: TableRecord): ParentRecords {
...
function detailTabs(record: TableRecord): readonly DetailTabItem[] {
  if (record.cell.kind === 'system') {
    return record.cell.previousPromptDetail === undefined ? SYSTEM_PROMPT_TABS : SYSTEM_UPDATE_TABS
  }
...
function toolCallTextParts(kind: TrajectoryCellKind, text: string): ToolCallTextParts | undefined {
...
function RecordPresentation({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: cell fields are listed individually to skip identity-only changes
...
function RecordListText({
  ...
  return (
    <>
      <span className={css.toolCallNameTypeface}>{toolCallText.name || '—'}</span>
      {toolCallText.args !== undefined && (
        <span className={css.toolCallPayload}>{toolCallText.args}</span>
      )}
    </>
  )
}
...
function MarkdownFragment({
  ...
  return <pre className={`${css.payload} ${preview ? css.payloadPreview : ''}`}>{text}</pre>
}
  ...
  return (
    <div className={css.sourceBlocks}>
      {blocks.map((block, index) => (
        // biome-ignore lint/suspicious/noArrayIndexKey: positional source blocks have no stable identity
        <section className={css.sourceBlock} key={index}>
          {block.callId !== undefined ? (
            ...
            >
              <span className={css.sourceBlockLabel}>{`Block #${index + 1} ${block.type}`}</span>
...
function PanelImage({
  ...
  return (
    <a
      className={
        preview ? `${css.panelImageLink} ${css.panelImageLinkPreview}` : css.panelImageLink
      }
      href={block.imageSrc}
      target="_blank"
      ...
      title="Open image"
    >
      <img className={css.panelImage} src={block.imageSrc} alt={block.imageAlt ?? ''} />
    </a>
  )
}
...
function MessageImages({
  ...
  return (
    <div
      className={preview ? `${css.messageImages} ${css.messageImagesPreview}` : css.messageImages}
    >
      {images.map((block, index) => (
        // biome-ignore lint/suspicious/noArrayIndexKey: positional image blocks have no stable identity
        ...
        <li key={call.callId ?? index}>
          ...
          >
            ...
            <span className={css.assistantToolCallText}>
              <span className={css.assistantToolCallName}>{call.toolName ?? 'tool-call'}</span>
              {call.content !== '' && (
        ...
        <details className={css.toolCatalogItem} key={`${tool.name}:${index}`}>
          ...
          <div className={css.toolCatalogDefinition}>
            ...
            {tool.source === 'registry' && (
              <p className={css.toolCatalogProvenance}>
                Definition from the current tool registry; this session recorded tool names only.
              </p>
            )}
            {tool.parameters === undefined ? (
              <p className={css.noPayload}>No parameter schema recorded</p>
            ) : (
              <JsonTree
    ...
    <section className={css.promptDiffSection}>
      ...
      <pre className={css.promptDiff}>
        {lines.map((line, index) => (
          // biome-ignore lint/suspicious/noArrayIndexKey: diff lines are positional render rows
  ...
  return (
    <div className={css.promptDiffSections}>
      {before.system !== after.system && (
        <PromptDiffSection title="System Prompt" before={before.system} after={after.system} />
      )}
      {toolsBefore !== toolsAfter && (
        <PromptDiffSection title="Tools" before={toolsBefore} after={toolsAfter} />
      )}
    </div>
  )
}
...
function ToolOutputBlocks({
  ...
  return (
    ...
    >
      {blocks.map((block, index) =>
        block.imageSrc !== undefined ? (
          // biome-ignore lint/suspicious/noArrayIndexKey: positional result blocks have no stable identity
          <PanelImage block={block} preview={preview} key={index} />
        ) : block.content !== '' ? (
          // biome-ignore lint/suspicious/noArrayIndexKey: positional result blocks have no stable identity
          <pre className={css.resultBlockText} key={index}>
            {block.content}
          </pre>
        ) : null,
      )}
...
function MarkdownRecordContent({
  ...
  if (record.cell.thinkingDetail) {
    if (!rendered) {
      const source = [record.cell.thinkingDetail, record.cell.outputDetail]
...
function RecordTiming({ record }: { record: TableRecord }) {
  return record.cell.kind === 'message' && record.cell.assistantMetrics !== undefined ? (
    ...
    <dl className={css.overview}>
      <div>
        <dt>Started</dt>
        <StartedAtValue timestamp={record.cell.startedAt ?? null} />
      </div>
      <div>
        <dt>Duration</dt>
        <dd>{formatElapsedSeconds(record.cell.timeSeconds)}</dd>
      </div>
      <div>
        <dt>Timing source</dt>
        <dd>{record.cell.timeSeconds === null ? 'Not available' : 'Session timestamps'}</dd>
      </div>
    </dl>
  )
}
...
function RequestTiming({
  ...
  if (request?.startedAt !== undefined) {
    const duration =
      request.completedAt === null || request.completedAt === undefined
        ? null
    ...
    return (
      <dl className={css.overview}>
        <div>
          <dt>Started</dt>
          <StartedAtValue timestamp={request.startedAt} />
        </div>
        <div>
          <dt>Duration</dt>
          <dd>{formatElapsedSeconds(duration)}</dd>
        </div>
        <div>
          <dt>Timing source</dt>
  ...
  return (
    <dl className={css.overview}>
      ...
      <div>
        <dt>Duration</dt>
        <dd>{formatElapsedSeconds(null)}</dd>
      </div>
    </dl>
  )
}
...
function RecordPayload({
  ...
  const singleTextResult =
    direction === 'output' &&
    record.cell.outputBlocks?.length === 1 &&
    record.cell.outputBlocks[0]?.type === 'text'
  if (singleTextResult && json !== undefined) {
    return <JsonTree data={json} label="Result JSON" className={payloadClassName} />
  }

  if (
    direction === 'output' &&
    record.cell.outputBlocks?.some(
      (block) => block.imageSrc !== undefined || block.content !== '',
    ) === true
  ) {
    return <ToolOutputBlocks blocks={record.cell.outputBlocks} error={error} preview={preview} />
  }

  const markdown =
    (direction === 'input' && (record.cell.kind === 'user' || record.cell.kind === 'context')) ||
...
export function TrajectoryTable({
  ...
  const requestInfoBySeq = useMemo(
    () => indexSessionRequestsBySeq(sessionRequestNumbers),
  ...
  const virtualBottom =
    virtualItems.length === 0
      ...
      : Math.max(
          0,
          rowVirtualizer.getTotalSize() + virtualScrollMargin - (virtualItems.at(-1)?.end ?? 0),
        )
    ...
    ? virtualItems.flatMap((item) => {
        ...
        return row.entries.map((entry) => ({
          ...
          terminalRequestBoundary:
            entry.record.cell.requestOnly === true && entry.logicalIndex === records.length - 1,
        }))
      })
  ...
  const selectedRequestInfo =
    selectedRequest === null
      ? undefined
      : sessionRequestNumbers?.find((request) =>
          selectedRequest.seq === undefined
            ? request.turn === selectedRequest.turn && request.group === selectedRequest.group
            : request.seq === selectedRequest.seq,
        )
  // A step group's assistant metrics belong to the request that anchored the
  // node, so match rows by seq; sibling passes or retries selected by seq must
  // not borrow each other's timing. Request-only rows carry no assistant
  // metrics, and selecting without a seq falls back to the group's first
  // content message row.
  const selectedRequestAssistant = selectedRequestRecords.find(
    (record) =>
      record.cell.kind === 'message' &&
      record.cell.requestOnly !== true &&
      (selectedRequest?.seq === undefined || record.cell.sourceSeq === selectedRequest.seq),
  )
  const selectedRequestAnchor =
    (selectedRequest?.seq === undefined
      ? undefined
      : selectedRequestRecords.find(
          (record) =>
            record.cell.requestOnly === true && record.cell.sourceSeq === selectedRequest.seq,
        )) ??
    selectedRequestAssistant ??
    selectedRequestRecords[0]
  const selectedRequestNumber =
    selectedRequest === null
      ? undefined
      : selectedRequest.seq === undefined
        ? requestNumbers.get(requestKey(selectedRequest.turn, selectedRequest.group))
        : (selectedRequestInfo?.number ??
          requestNumbers.get(requestKey(selectedRequest.turn, selectedRequest.group)))
  const selectedRequestState: RecordState | undefined =
    selectedRequest === null
      ? undefined
  ...
  const selectedRequestResultTemplate =
    selectedRequestInfo?.resultSeq === undefined
      ? selectedRequestAssistant
      : allRecords.find((record) => record.cell.sourceSeq === selectedRequestInfo.resultSeq)
  const selectedRequestResult =
    selectedRequestResultTemplate === undefined
      ? undefined
      : currentRecord(selectedRequestResultTemplate)
  const selectedRequestUsage =
    selectedRequestInfo?.usage ??
    (selectedRequestAssistant === undefined
      ? undefined
      : {
  ...
  const activeSection =
    selectedRequest === null ? selected?.section : selectedRequestRecords[0]?.section
  const selectedTabs =
    selectedRequest !== null
      ...
      : selected === undefined
        ? []
        : detailTabs(selected)
  const selectedParents: ParentRecords =
    selected === undefined ? {} : parentRecords(allRecords, selected)
  const selectedParentMessage = selectedParents.message
  const selectedParentTool = selectedParents.tool
  const selectedAssistantRequest =
    selected?.cell.kind === 'message'
      ? requestNumbers.get(requestKey(selected.turn, selected.group))
      : undefined
  const selectedAssistantRequestInfo =
    selectedAssistantRequest === undefined
      ? undefined
  ...
  useEffect(() => {
    ...
    pendingScrollRecordId.current = record === undefined ? null : trajectoryRecordId(record.cell)
  ...
  const selectRequest = (request: SelectedRequest, tab: 'overview' | 'timing' = 'overview') => {
    setSelectedRecordId(null)
  ...
  useEffect(() => {
    ...
    const position = records.findIndex(
      (record) => trajectoryRecordId(record.cell) === id && record.collapsedSummary === undefined,
    )
  ...
  useEffect(() => {
    ...
    if (!virtualizationEnabled) {
      ...
      const target =
        focusHeight > ledger.clientHeight
          ? firstRow
    ...
    const focusedVirtualIndexes = [
      ...new Set(
        focusedPositions.flatMap((position) => {
  ...
  const requestOlder = useCallback(
    (pane: HTMLDivElement, requireTop: boolean) => {
      if (
        !hasOlderRecords ||
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: scroll anchor only shifts when history paging state changes
  useLayoutEffect(() => {
  ...
  }, [historyLoading, historyStartSeq, rowVirtualizer, virtualRowStructure, virtualizationEnabled])
  ...
  return (
    <div ref={rootRef} className={css.split} style={splitStyle}>
      {/* biome-ignore lint/a11y/useKeyWithClickEvents: background click clears selection; keyboard users clear via row handlers */}
      <div
        ref={tablePaneRef}
      ...
      >
        {showInitialLoading && (
          // biome-ignore lint/a11y/useSemanticElements: vendored live region; role=status on a plain div is intentional
        ...
        >
          ...
          <tbody>
            {hasOlderRecords && (
              <tr className={css.historyLoadRow} data-history-load="" aria-rowindex={1}>
                <td colSpan={2}>
                  <button
                    type="button"
                  ...
                  >
                    {olderBusy && <span className={css.historyLoadingSpinner} aria-hidden="true" />}
                    <span aria-hidden="true">
                      {olderBusy ? 'Loading earlier history…' : 'Load earlier history'}
                    </span>
                    {/* biome-ignore lint/a11y/useSemanticElements: visually-hidden live region, vendored pattern */}
            ...
            {virtualTop > 0 && (
              // biome-ignore lint/a11y/noAriaHiddenOnFocusable: virtualization spacer row is inert decoration
            ...
            {renderedRecords.map(({ record, position, terminalRequestBoundary }) => (
              <RecordPresentation key={trajectoryVirtualRecordKey(record)} cell={record.cell}>
                {({ displayText, listDisplayText, resultText, toolCallOnly, toolCallText }) => {
                  ...
                  const key = requestKey(record.turn, record.group)
                  // Rows carry the seq of the request they anchor (startSeq
                  // === node seq), step-0 content included via its Message
                  // group row; resolving by seq keeps each request numbered on
                  // its own content row instead of a group-wide boundary
                  // number or a separate request-only row.
                  const ownRequestInfo =
                    record.cell.sourceSeq !== undefined
                      ? requestInfoBySeq.get(record.cell.sourceSeq)
                      : undefined
                  const request =
                    !isCollapsedSummary &&
                    (record.turn === null || !collapsedTurns.has(record.turn))
                      ? (ownRequestInfo?.number ??
                        (requestBoundaries.get(key) === record.cell.index
                  ...
                  const requestStatus =
                    requestInfo?.status ?? (record.cell.isError === true ? 'error' : undefined)
                  ...
                  const requestSelected =
                    ...
                    (selectedRequest.seq === undefined ||
                      requestInfo?.seq === undefined ||
                      requestInfo.seq === selectedRequest.seq)
                  const sectionActive =
                    record.turn === null
                  ...
                  return (
                    <tr
                      ...
                      aria-selected={
                        !isCollapsedSummary && !isRequestOnly && selectedIndex === record.cell.index
                      }
                      data-kind={record.cell.kind}
                      data-assistant-message-id={
                        record.cell.kind === 'message' ? record.cell.recordId : undefined
                      }
                      data-trajectory-row-key={trajectoryVirtualRecordKey(record)}
                      data-virtual-position={virtualizationEnabled ? position : undefined}
                      data-record-index={
                        !isCollapsedSummary && !isRequestOnly ? record.cell.index : undefined
                      }
                      ...
                      data-selected={
                        (!isCollapsedSummary && selectedIndex === record.cell.index) || undefined
                      }
                      data-timeline-focus={
                        isCollapsedSummary || timelineFocusIndexes === null
                          ? undefined
                          : timelineFocusIndexes.has(record.cell.index)
                            ? 'inside'
                            : 'outside'
                      ...
                      onClick={
                        isRequestOnly
                          ...
                          : isCollapsedSummary
                            ? () => {
                                if (
                                  record.collapsedSummaryKind === 'turn' &&
                                  record.turn !== null
                                ) {
                    ...
                    >
                        ...
                        <div className={css.eventInner}>
                          {!isCollapsedSummary && (
                            <span className={css.kindSlot}>
                              ...
                              >
                                <Tooltip label={recordBadgeLabel(record)} side="right">
                                ...
                                <span className={css.kindTagLabel}>{recordBadgeLabel(record)}</span>
                              </span>
                            </span>
                          )}
                        </div>
                      ...
                      <td className={css.content}>
                        {record.collapsedSummary !== undefined ? (
                          ...
                          <span
                            className={
                              resultText === undefined ? css.contentText : css.resultPreview
                            }
                            title={
                          ...
                          >
                            <span
                              className={resultText === undefined ? undefined : css.resultRequest}
                            >
                              <RecordListText
        ...
        >
          <div
            ...
            onKeyDown={(event) => {
              ...
              const currentToolRequestOffset =
                toolRequestOffset ??
                splitWidth * TOOL_REQUEST_SHARE - defaultToolRequestWidth(splitWidth)
              setDetailsWidth(nextDetailsWidth)
              setToolRequestOffset(
          ...
          <div className={css.detailTabs} role="tablist" aria-label="Event details">
            {selectedTabs.map((tab) => (
              <button
                ...
                className={
                  activeTab === tab.id ? `${css.detailTab} ${css.detailTabActive}` : css.detailTab
                }
                onClick={() => {
                  activateTab(tab.id)
                }}
          ...
          >
            ...
            {promptSelected && selectedPreviousPrompt !== undefined && activeTab === 'diff' && (
                ...
                <>
                  ...
                  >
                    ...
                    {hasSelectedHierarchy && (
                      <div>
                        <dt>
                          {selectedAssistantRequestTarget !== undefined ? 'Source' : 'Hierarchy'}
                        </dt>
                        <dd className={css.overviewParentLinks}>
                          {selectedAssistantRequestTarget !== undefined && (
                            <button
                    ...
                    {selected.cell.kind === 'tool' &&
                      selected.cell.evidenceDetail !== undefined && (
                        <div>
                          <dt>Evidence</dt>
                          <dd>
                            {selected.cell.evidenceDetail.path}
                            {selected.cell.evidenceDetail.chars !== undefined
                              ? ` · ${selected.cell.evidenceDetail.chars.toLocaleString()} chars`
                              : ''}
                            {selected.cell.evidenceDetail.sha256
                              ? ` · sha256 ${selected.cell.evidenceDetail.sha256.slice(0, 12)}`
                              : ''}
                          </dd>
                        </div>
                      )}
                    {selected.cell.kind === 'message' && <TokenRows cell={selected.cell} />}
                    {isGateQaRecord(selected) && <GateTokenRows cell={selected.cell} />}
