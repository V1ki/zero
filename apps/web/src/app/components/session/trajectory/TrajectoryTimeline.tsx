...
function timelineRecordDetail(cell: TrajectoryCellProps): TimelineRecordDetail {
  ...
  const startedAt =
    cell.startedAt === null || !Number.isFinite(cell.startedAt) ? undefined : cell.startedAt
  return {
...
function timelineKindLabel(kind: TrajectoryBadgeKind): string {
  switch (kind) {
    case 'system':
      return 'SYSTEM'
    case 'user':
...
export const TrajectoryTimeline = memo(function TrajectoryTimeline({
  ...
  useEffect(() => {
    ...
    setViewport((current) => {
      ...
      const desiredStart =
        selectedSpan.end <= current.start ? selectedSpan.start : selectedSpan.end - duration
  ...
  const recordIndexAt = (event: PointerEvent<HTMLDivElement>): number | null => {
    ...
    const value = target?.closest<HTMLElement>('[data-timeline-record-index]')?.dataset
      .timelineRecordIndex
          ...
          >
            {model.spans
              ...
              .map((span) => {
                ...
                const ttftFraction =
                  ttftMs === undefined || decodingMs === undefined || ttftMs + decodingMs <= 0
                    ? null
