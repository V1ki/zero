...
import { cellBadgeKind, formatDurationMillis } from './trajectory-record'
import type { TrajectoryBadgeKind, TrajectoryCellProps } from './trajectory-record'
...
function laneFor(kind: TrajectoryBadgeKind): number {
    ...
    spans.push(
      ...cells.map(
        (cell, offset): TrajectoryTimelineSpan => ({
          ...
          kind: cellBadgeKind(cell),
          label: cell.text,
          lane: laneFor(cellBadgeKind(cell)),
        }),
      ),
    )
  }
