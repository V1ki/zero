import type { TrajectoryCellProps, TrajectorySourceBlock } from './trajectory-record'
import { formatElapsedSeconds } from './trajectory-record'
/**
...
/**
 * Detail fields for a gate record that captured a question/answer pair: the
 * question becomes the record's Payload and the answer its Result, mirroring
 * tool records, while the row preview stays the gate headline.
 * @param node - Context node carrying a gateQa pair.
 * @param gateQa - Captured question/answer.
 * @returns Tool-style cell detail fields.
 */
function gateQaCellDetail(
...
> {
  ...
  return {
    ...
    sourceSeq: node.seq,
    messageSource: node.source,
    ...(gateQa.question === '' ? {} : { inputDetail: gateQa.question }),
    outputDetail: gateQa.answer,
    outputBlocks: gateQa.answer === '' ? [] : [{ type: 'text', content: gateQa.answer }],
    sourceBlocks: node.content.map((block) => sourceBlock(block)),
    timeSeconds: gateQa.durationMs === undefined ? 0 : gateQa.durationMs / 1000,
    startedAt: finiteTime(gateQa.startedAt ?? node.time),
    ...(gateQa.usage?.input === undefined ? {} : { input: gateQa.usage.input }),
    ...(gateQa.usage?.cacheRead === undefined ? {} : { cacheRead: gateQa.usage.cacheRead }),
    ...(gateQa.usage?.cacheWrite === undefined ? {} : { cacheWrite: gateQa.usage.cacheWrite }),
    ...(gateQa.usage?.output === undefined ? {} : { output: gateQa.usage.output }),
    ...(gateQa.usage?.reasoning === undefined ? {} : { think: gateQa.usage.reasoning }),
  }
}
...
export function deriveTrajectoryLayout(
  ...
  // Requests anchor at the assistant node carrying their content (startSeq is
  // the node seq), step-0 nodes included — their rows render in the Message
  // group. Anchored requests keep their chip on the content row; only
  // anchor-less requests — true retries and nudge requests without output —
  // need request-only rows. In-flight steps (partial or running calls) stay
  // row-less until they anchor.
  const anchoredRequestSeqs = new Map<string, Set<number>>()
  const anchorStepSeq = (turn: number, step: number, seq: number) => {
    const key = `${turn}\u0000${step}`
    const seqs = anchoredRequestSeqs.get(key) ?? new Set<number>()
    seqs.add(seq)
    anchoredRequestSeqs.set(key, seqs)
  }
  for (const node of nodes) {
    if (node.kind === 'assistant') {
      anchorStepSeq(node.turn, node.step, node.seq)
    }
  }
  const streamingSteps = new Set<string>()
  if (partial !== null && partial.step > 0) {
    streamingSteps.add(`${partial.turn}\u0000${partial.step}`)
  }
  for (const call of runningCalls) {
    if (call.step > 0) streamingSteps.add(`${call.turn}\u0000${call.step}`)
  }
  ...
  const entries: OrderedLayoutEntry[] = [
    ...
    ...requests
      .filter((request): request is CompactionRequestView => request.purpose === 'compaction')
      .map((request) => ({
    ...
    ...requests
      .filter((request): request is AssistantRequestView => request.purpose === 'assistant')
      .filter((request) => {
        const key = `${request.turn}\u0000${request.step}`
        if (anchoredRequestSeqs.get(key)?.has(request.startSeq)) return false
        return !streamingSteps.has(key)
      })
      .map((request) => ({
    ...
    if (node.kind === 'context') {
      ...
      pushMessage(turn, {
        ...
        cell: {
          index: ++index,
          kind: 'context',
          ...(node.gateQa === undefined
            ? inputCellDetail(node)
            : gateQaCellDetail(node, node.gateQa)),
        },
      })
      prevAbsTime = finiteTime(node.time) ?? prevAbsTime
      continue
        ...
        const laidList: LaidCell[] = [
          {
            ...
            cell: {
              ...
              ...(node.evidence ? { evidenceDetail: node.evidence } : {}),
  ...
  if (partial !== null) {
    ...
    const laidList = withSubCalls(
      expandAssistant(fake, index + 1, prevAbsTime, resultByCall, callStartById, callById, {
        streaming: true,
      }),
    )
...
function toTurnModel(turn: number | null, entry: TurnBucket): TrajectoryTurnModel {
...
function expandAssistant(
  ...
  const message: TrajectoryCellProps = {
    index: ++index,
    recordId: node.messageId ?? `assistant\u0000${node.turn}\u0000${node.step}`,
    kind: 'message',
    sourceSeq: node.seq,
  ...
  for (const block of node.blocks) {
    ...
    const toolDuration =
      streaming || result === undefined ? null : durationSeconds(result.time, result.callTime)
      ...
      cell: {
        ...
        ...(result !== undefined
          ? {
              ...
              ...(result.evidence ? { evidenceDetail: result.evidence } : {}),
            }
          : {}),
...
function sourceImage(block: Record<string, unknown>): string | undefined {
  ...
  if (typeof block.data === 'string') {
    ...
    return safeImageSource(
      block.data.startsWith('data:') ? block.data : `data:${mediaType};base64,${block.data}`,
    )
  }
...
function previewContent(content: readonly { type: string; text?: string }[]): string | undefined {
