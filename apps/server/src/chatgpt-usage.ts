...
interface ChatGptRawRateLimitStatusDetails {
  primary_window?: ChatGptRawRateLimitWindow | null
  secondary_window?: ChatGptRawRateLimitWindow | null
}

interface ChatGptRawCreditsSnapshot {
  has_credits?: boolean
  unlimited?: boolean
  balance?: string | null
}

interface ChatGptRawAdditionalRateLimit {
  limit_name?: string
  metered_feature?: string
  rate_limit?: ChatGptRawRateLimitStatusDetails | null
}

interface ChatGptRawUsagePayload {
  plan_type?: string
  rate_limit?: ChatGptRawRateLimitStatusDetails | null
...
function toWindow(window: ChatGptRawRateLimitWindow | null | undefined): ChatGptUsageWindow | null {
...
function toRateLimitSnapshot(params: {
  limitId: string | null
  limitName: string | null
  ...
  return {
    ...
    primary: toWindow(params.rateLimit?.primary_window),
    secondary: toWindow(params.rateLimit?.secondary_window),
    credits: toCredits(params.credits),
    planType: params.planType,
  }
}

function normalizeUsagePayload(payload: ChatGptRawUsagePayload): ChatGptUsageSnapshot {
  const planType = typeof payload.plan_type === 'string' ? payload.plan_type : null
  const root = toRateLimitSnapshot({
    limitId: 'codex',
    limitName: null,
  ...
  for (const item of payload.additional_rate_limits ?? []) {
    ...
    byLimitId[limitId] = toRateLimitSnapshot({
      limitId,
