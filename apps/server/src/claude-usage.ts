...
function hasProfileScope(scopes: string[]) {
  return scopes.includes('user:profile')
}

function isClaudeSubscriber(subscriptionType: string | null | undefined) {
...
export class ClaudeUsageService {
  ...
  async fetchUsage(): Promise<ClaudeUsageSnapshot | null> {
    ...
    if (!isClaudeSubscriber(session.subscriptionType) || !hasProfileScope(session.scopes)) {
      return {}
    }
